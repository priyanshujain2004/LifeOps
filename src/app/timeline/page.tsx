...
export default function TimelinePage() {
  ...
  return (
    <div className="space-y-6 pb-6 animate-fade-in">
      {/* Date Bar & Header */}
      <div className="p-5 rounded-3xl bg-slate-900 border border-slate-800 shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/15 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2 border border-indigo-500/30">
            <Clock className="w-3.5 h-3.5" /> Chronological Activity Ledger
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white">Daily Timeline View</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Server-stamped immutable logs formatted in <span className="font-mono font-bold text-indigo-300">IST (Asia/Kolkata)</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-initial">
            <CalendarIcon className="w-4 h-4 text-indigo-400 absolute left-3 top-3 pointer-events-none" />
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="w-full sm:w-auto rounded-xl bg-slate-950 border border-slate-700 pl-9 pr-3 py-2 text-xs font-mono font-bold text-slate-100 focus:outline-none focus:border-indigo-500 shadow-inner"
            />
          </div>
          ...
          {selectedDate !== getTodayIST() && (
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 text-xs font-bold border border-indigo-500/30 transition-colors flex items-center gap-1 shrink-0"
            >
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="p-12 rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 text-center space-y-2">
            ...
            <h3 className="text-sm font-semibold text-slate-300">No logs recorded on {selectedDate}</h3>
