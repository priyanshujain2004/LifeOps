      ...
      <div className="w-full max-w-lg rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-2xl text-slate-100 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-lg leading-tight">Log Expense Entry</h3>
              <p className="text-xs text-slate-400">Tie expense to trip session or standalone activity</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full rounded-xl bg-slate-950 border border-slate-800 pl-8 pr-3.5 py-2.5 text-sm font-mono text-slate-100 focus:outline-none focus:border-indigo-500 font-bold"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl bg-slate-950 border border-slate-800 px-3.5 py-2.5 text-sm text-slate-100 font-semibold focus:outline-none focus:border-indigo-500"
              >
          ...
          {/* Description */}
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Description / Vendor</label>
            <input
              type="text"
              required
              value={description}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Link to Trip Session (Optional)</label>
            <select
              value={selectedTripId}
              onChange={(e) => handleTripChange(e.target.value)}
              className="w-full rounded-xl bg-slate-950 border border-slate-800 px-3.5 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500"
            >
          ...
          <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className={`p-2 rounded-lg ${reimbursable ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-800 text-slate-400"}`}>
                {reimbursable ? <CheckCircle2 className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
              </div>
              <div>
                <h4 className="text-sm font-bold text-slate-200">Reimbursable from Company?</h4>
          ...
          <div>
            ...
            <div className="relative border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl p-4 text-center transition-colors cursor-pointer bg-slate-950/40">
              <input
                type="file"
                accept="image/*"
              ...
              {previewUrl ? (
                <div className="flex items-center gap-3">
                  <img src={previewUrl} alt="Receipt Preview" className="w-14 h-14 object-cover rounded-lg border border-slate-700" />
                  <div className="text-left">
                    <p className="text-xs font-semibold text-emerald-400 truncate max-w-[220px]">{receiptFile?.name}</p>
                    <p className="text-[10px] text-slate-400">Tap or drag new image to replace</p>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center gap-1 py-2 text-slate-400">
                  <Upload className="w-6 h-6 text-indigo-400 mb-1" />
                  <p className="text-xs font-semibold text-slate-200">Upload Receipt Photo</p>
