    ...
    <ModalPortal isOpen={isOpen} onClose={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl text-slate-900 dark:text-slate-100 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                <span className="flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> Origin
                </span>
              </label>
              <select
                value={originId}
              ...
              {!originId && (
                <input
                  ...
                  className="w-full mt-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-3 py-1.5 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
                />
              )}
            </div>
          ...
          {showAddLocation && (
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-indigo-500/40 space-y-2.5 animate-fade-in">
              <h4 className="text-xs font-bold text-indigo-600 dark:text-indigo-300 flex items-center justify-between">
                <span>Save New Location for Autocomplete</span>
                <button type="button" onClick={() => setShowAddLocation(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-3 py-1.5 text-xs text-slate-900 dark:text-slate-100"
                />
                <select
                  value={newLocType}
                  onChange={(e) => setNewLocType(e.target.value as any)}
                  className="rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2 py-1.5 text-xs text-slate-900 dark:text-slate-100 font-semibold"
                >
          ...
          {/* Optional Notes */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-400 mb-1">Trip Notes (Optional)</label>
            <input
              type="text"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="e.g. Traveling with engineering team for inspection"
              className="w-full rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-3.5 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
            />
          </div>
