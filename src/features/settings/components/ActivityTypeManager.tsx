...
import { Plus, Edit3, Trash2, ArrowUp, ArrowDown, Check, X, Sparkles, Zap, DollarSign, Save } from "lucide-react";
...
export function ActivityTypeManager({ activityTypes, onSave, onDelete, onMoveSort }: ActivityTypeManagerProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Editor Form */}
      {editingId && (
        <form onSubmit={handleSubmit} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-indigo-500/60 space-y-4 animate-fade-in shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
            ...
            <button type="button" onClick={() => setEditingId(null)} className="p-1 rounded text-slate-400 hover:text-slate-900 dark:hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Button Name</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Category Group</label>
              <select
                value={category}
            ...
            <div className="flex gap-2">
              <div className="w-1/2">
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Emoji Icon</label>
                <input
                  type="text"
                  maxLength={4}
                  value={icon}
                  onChange={(e) => setIcon(e.target.value)}
                  className="w-full rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-2 py-2 text-center text-base text-slate-900 dark:text-slate-100"
                />
              </div>
              <div className="w-1/2">
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Badge Color</label>
                <input
                  type="color"
                  value={color}
                  onChange={(e) => setColor(e.target.value)}
                  className="w-full h-9 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 cursor-pointer p-1"
                />
              </div>
            </div>
          </div>
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <input
                  ...
                  className="rounded bg-slate-100 dark:bg-slate-900 border-slate-300 dark:border-slate-700 w-4 h-4 text-indigo-600"
                />
              </div>
              {isPaired && (
                <input
                  ...
                  className="w-full rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2.5 py-1.5 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-500"
                />
              )}
            </div>
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <input
                  ...
                  className="rounded bg-slate-100 dark:bg-slate-900 border-slate-300 dark:border-slate-700 w-4 h-4 text-emerald-600"
                />
              </div>
              {isExpenseTrigger && (
                <select
                  ...
                  className="w-full rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2 py-1.5 text-xs text-slate-900 dark:text-slate-100 font-mono"
                >
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              className="px-4 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow flex items-center gap-1.5"
            >
              <Save className="w-3.5 h-3.5" />
              <span>{editingId === "NEW" ? "Save Button" : "Update Button"}</span>
            </button>
          </div>
        </form>
      )}
